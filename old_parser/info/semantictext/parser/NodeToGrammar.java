...
public class NodeToGrammar
{
    ...
    {
        // Check definitions
        if (!node.getCanonicalName().equals("namespace_definition"))
        {
    ...
    private static GType createGType(Node node, String namespace) throws ParseException
    {
        ...
        for (Node n: childs)
        {
            if (n.getCanonicalName().equals("name"))    updateName(result, n.getValue().trim());
            if (n.getCanonicalName().equals("alias"))   updateAlias(alias, n.getValue().trim());
            if (n.getCanonicalName().equals("type"))    updateType(result, n.getValue().trim());
            if (n.getCanonicalName().equals("child"))   updateChild(ccc, n, namespace);
        }        
    ...
    private static void updateChild(List<GTypeChild> ccc, Node n, String namespace)
    {
        ...
        {
            if (child.getCanonicalName().equals("num"))         result.setNum(child.getValue().trim());
            if (child.getCanonicalName().equals("namespace"))   result.setNamespace(child.getValue().trim());
            if (child.getCanonicalName().equals("type"))        result.setType(child.getValue().trim());
        }
